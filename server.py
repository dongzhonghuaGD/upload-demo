...
def upload_success():  # 按序读出分片内容，并写入新文件
    ...
    #         问题就是如果按照他说的那样不合并文件，那么我想的就是可以新建一个文件夹，文件命名，里面的分块可以不合并，、
    #         这样可以下载时遍历这个文件夹然后就可以一次yield
    #         但是又出现了另一个问题那就是在listfile的时候就会特别混乱，当然改也是可以的，但是得不偿失。
    #         当然没有这个功能也可以仔细的看看源代码，特别是可以看看HTML和js部分的实现
    return rt('./index.html')
